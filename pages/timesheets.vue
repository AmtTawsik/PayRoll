...
const composable = await import("~/composables/timesheet");
const piniaStore = await import("~/stores/timesheet");
...
tableConfig.tableName = "timesheet";
tableConfig.headers = config;

tableConfig.rowMap = new Map();
tableConfig.groupMap = new Map();

tableConfig.hideComposable = composable.useTimesheetHideDropDown;
tableConfig.groupComposable = composable.useTimesheetGroup;
tableConfig.changeGroup = changeGroup;
tableConfig.currentRecord = composable.useTimesheet;

tableConfig.store = piniaStore.useStore;
tableConfig.drawerComponent = resolveComponent("TimesheetDrawer");
...
onBeforeMount(async () => {
	...
	saveViewList.value = await getRecords("payroll", "timesheets");
...
<template>
	<div v-if="store.recordsData?.length > 0" class="flex flex-col h-full px-5 pt-5">
		...
		<RecordsTable :key="tableKey" :TABLE_DATA="store.recordsData" :tableConfig="tableConfig"
