...
const composable = await import("~/composables/agency");
const piniaStore = await import("~/stores/agency");
...
tableConfig.tableName = "agency";
tableConfig.headers = config;

tableConfig.rowMap = new Map();
tableConfig.groupMap = new Map();

tableConfig.hideComposable = composable.useAgencyHideDropDown;
tableConfig.groupComposable = composable.useAgencyGroup;
tableConfig.changeGroup = changeGroup;
tableConfig.currentRecord = composable.useAgency;

tableConfig.store = piniaStore.useStore;
tableConfig.drawerComponent = resolveComponent("AgencyDrawer");
...
onBeforeMount(async () => {
	...
	// saveViewList.value = await getRecords("payroll", "agencies");
